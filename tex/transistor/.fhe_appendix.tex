...
\else{
	...
	The \coolName design involves two LSFRs operating on elements of $\field{17}$. The standard way to implement an \gls{LFSR} is to evaluate the linear feedback function on the state at each clock cycle, thus producing a new element that enters the state, while the state is shifted to output an element. 
	...
	We suggest the \emph{silent \gls{LFSR}} approach for the homomorphic evaluation of LFSRs. In this approach, the encrypted \gls{LFSR} state is immutable to avoid any noise growth in the underlying ciphertexts (hence keeping the \gls{LFSR} ``silent''). We use the fact that every output element of the \gls{LFSR} can be expressed as a linear combination of the initial state. So, at each clock cycle, we compute \emph{in the clear} the coefficients of this linear combination and homomorphically evaluate it on the immutable encrypted state. This process is depicted in Algorithm~\ref{alg:lsfr}.
	
	\input{algorithms/silent_lsfr.tex}
	
	%Calling Algorithm \ref{alg:lsfr} (\texttt{\gls{LFSR}.clock}) several times yields an encrypted stream of pseudorandom elements $(o^{(0)}, o^{(1)}, \dots)$.
