...
\newcommand\Kfhe{K_\mathsf{\gls{FHE}}}
\newcommand\Kspacesym{\mathcal{K}_\mathsf{sym}}
\newcommand\Kspacefhe{\mathcal{K}_\mathsf{\gls{FHE}}}
  ...
  \caption{\label{fig:transciphering-principle} The principle of transciphering, where $E$ is a symmetric cipher (with secret key $\Ksym$ sampled from the space $\Kspacesym$), $H$ is a fully homomorphic cipher (with private key $\Kfhe$ sampled from the space $\Kspacefhe$), $E^H$ is a homomorphic evaluation of $E$, $A$ corresponds to some arbitrary operations, and $A^H$ to their homomorphic evaluation. %\nicolas{Il y a un souci avec la troisième ligne du serveur non ? Ca n'a pas de sens rechiffrer avec Transistor les chiffrés \gls{TFHE}. Pour le sens "retour" les techniques sont différentes (cf eprint.iacr.org/2024/1921 par exemple). Je pense qu'il vaut mieux ne pas le mettre}\lp{c'est implémenté}
    ...
    %\nicolas{J'ai rajouté la génération de clé \gls{FHE}; et j'ai remplacé par des notations génériques pour els espaces de clé (on n'a pas encore introduit que Transistor marche dans $\mainField$)}
    %\gl{Si on veux ajouter la clef \gls{FHE} dans le schema, il faut transferer la clef publique, non?}
  }
