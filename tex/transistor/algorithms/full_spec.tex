...
\begin{equation*}
  ...
  \end{split}
\end{equation*}
and for the whitening \gls{LFSR} $\whitening$ we use
\begin{equation*}
  \footnotesize
  \begin{split}
...
\item Then, while keystream blocks are needed, we repeat the following:
  ...
  \item ($\phi$) Take the elements with indices in $\{ 4, 6, 12, 14\}$, clock the whitening \gls{LFSR} four times, and add its successive outputs to these elements. The result forms a keystream tuple of four digits.
