...
\TODO{Relire ça et en faire quelque chose de sensé. J'ai écrit automatiquement}

The initial appearance of the notion of homomorphic encryption in the scientific literature can be traced back to 1978, in a paper by Rivest, Adleman and Dertouzos \cite{RAD78}. They theorize the existence of ``privacy homomorphism'', which are ``encryption functions which permit encrypted data to be operated on without preliminary decryption''. 

The following decades saw the development of \textit{partially} homomorphic encryption schemes, that allow to evaluate homomorphically only one mathematical operation. The targeted operation was often addition, and the application was for electronic voting schemes. The most well-known example of such scheme is the Paillier, introduced in \cite{}.


Another class of restricted homomorphic scheme was developed in parallel: those are \textit{leveled} (or \textit{somewhat}) homomorphic schemes. With those, both additive and multiplicative homomorphisms are available, but only a bounded number of these operations can be performed in a row. Example of those schemes include DHGV \cite{EC:DGHV10} and the original BGV scheme\footnote{These schemes have actually be shown to be bootstrappable, but the bootstrapping operation were too slow to be actually used in practice.} \cite{bgv}

Interestingly, two different phenomenons explain why these schemes were only partially homomorphic, but actually carry the same metaphorical signification: in DHGV the integers carrying the ciphertexts becomes bigger and bigger as homomoprhic operations until becoming untractable, while in BGV it is an additional Gaussian noise that grows until it detroys the information in the ciphertexts. In both case, a growing metric made impossible to evaluate unbounded computational circuits.



The story of \textit{actual} FHE begins in 2009, with a breakthrough contribution by Craig Gentry that changeed everything in the field. We explain it in next section.
...
We should also mention BFV/BGV \cite{C:Brakersky12, bgv, EPRINT:FanVer12}, that share most of the properties of CKKS. However these constructions do not work with approximate floating-point computation, but rather with exact computations over large integer rings.
