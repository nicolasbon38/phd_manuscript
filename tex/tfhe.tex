...
\TODO{Ecrire une petite page qui présente TFHE rapidement, et notamment explique le "plan" ed la section et les features du bootstrapping}
...
In Definition \ref{def:LWE}, we did not precise the shapes of the distributions $\chi_s$ and $\chi_e$ (beyond the fact they yields small values). Several distributions are possible: a discrete Gaussian with a small variance, a uniform distribution restricted on a small interval, or a binomial. The choice of the right distribution depends of the considered use-case, each possibilities offering different trade-offs in matter of security and efficiency.
\TODO{Trouver des référenes, par ex : eprint/2024/1979 ou eprint/21/1358 or eprint 2019/389, liste de travaux de Rachel Player}

Most implementations of TFHE select a uniform distribution on $\{0, 1\}$ for the secret, and a Gaussian with a small variance $\lweSigma$ for the noise. We will use those distributions in this thesis, and will use the notation $\LWE_{(q, n, \sigma)}$ for these instances.
...
\begin{definition}($\LWE$ ciphertext)
	...
	\begin{itemize}
		...
		\TODO{Confusion de notation avec le modsiwtch ?}
...
\TODO{Vérifier que toutes les décompositions et leurs définitions sont cohérentes entre ellesx}
...
We can now move to the most important feature of TFHE: the \textit{programmable bootstraapping}.
...
It is simply done by replacing the coefficients of the accumulator by the evaluations of a function $f:\plaintextRing \mapsto \plaintextRing$. So, the algorithm outputs $f(i)$ instead of $i$ ! This is illustrated on Figure \ref{fig:illustration_accumulator}.
...
Figure \ref{fig:PBS_layout} sums up the ciphertexts type, their shape and the noise behaviour during the PBS algorithm. It is presented as a loop, to show that it is possible to chain several PBS. Linear operations can be interleaved in the graph to evaluate any computational circuit, on the figure they are before the $\KeySwitch$ but they can also be placed after it.
\TODO{Ce paragraphe est mal écrit}
...
\begin{figure}
	\centering
	\includegraphics[width=0.8\linewidth]{img/timings_pbs_precisions.png}
	\label{fig:PBS_perfs}
...
Usually, PBS is not used for plaintext modulus greater than 8 bits. Some constructions exist in the literature to extend the capabilities of this algorithm to larger plaintext spaces, notably \cite{bibid}. In this thesis, we propose our own in Chapter \ref{chap:larger_lut}.

\TODO{Ici faire un petit survey, mais ne pas se répéter avec la section suivante}
