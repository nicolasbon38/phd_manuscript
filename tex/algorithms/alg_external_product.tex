\begin{algorithm}
	...
	\KwIn{
		$\left\{
		\begin{array}{l}
		\mat C_1 = \left \lbrace \vec K_\alpha = \left ( \GLWE_{\glweSecretKey}\left (-S_\alpha \cdot M \cdot \frac{q}{B^j} \right) \right )_{0 \le j < \levelDecomp} \right \rbrace_{0\le \alpha \le k+1} \text{: a ciphertext } \GGSW_{\glweSecretKey}(M)\\
		\vec C_2 = (A_0, \dots, A_{k-1}, B) \text{: a ciphertext }\GLWE_{\glweSecretKey}(M_1) \\
		\end{array}
		\right.$
	}
