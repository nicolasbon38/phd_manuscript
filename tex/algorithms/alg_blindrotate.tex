\begin{algorithm}
	...
	\KwContext{
		...
		\begin{array}{l}
			...
			acc(X) \in \glweRing \text{: an accumulator polynomial encoding the function } $f$.
		\end{array}
		\right.$
	}
	...
	\KwIn{
		$\left\{
		\begin{array}{l}
			\tilde c \text{: a modswitched $\LWE$ ciphertext}\\
	...
	\KwResult{
		$C_{\textsf{out}} = GLWE_{\glweSecretKey}(acc(X) \cdot X^{-m})$
	}
	...
	$C_{\textsf{out}} \gets \TrivialGLWE(acc(X) \cdot X^{-\tilde b})$\\
